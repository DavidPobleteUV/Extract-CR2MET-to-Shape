...
DATA_DIR  = os.path.join(BASE, "Results", "Aconcagua_EmbCatemu")
...
AREA_DBF  = os.path.join(BASE, "Cuenca", "Aconcagua_EmbCatemu",
                         "Aconcagua_EmbCatemu_mejorado.dbf")
...
DRY_RUN = True       # <-- set to False to actually modify the model
...
def main():
    ...
    for catch, bands in catchments.items():
        ...
        for band, label in bands:
            child_name = band if band else label
